...
const categories = ["Study Material", "Previous Papers", "Notes", "Syllabus", "Admit Card", "Other"];
...
const DownloadsManagement = () => {
  ...
  const resetForm = () => {
    setFormData({ title: "", description: "", file_url: "", category: "Study Material", is_published: false });
    setEditingItem(null);
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const fileName = `downloads/${Date.now()}.${fileExt}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      const { error } = await supabase.from("downloads").update(payload).eq("id", editingItem.id);
      if (error) {
        toast({ title: "Error", description: "Failed to update", variant: "destructive" });
      } else {
        toast({ title: "Success", description: "Download updated" });
      ...
      if (error) {
        ...
        toast({ title: "Success", description: "Download added" });
        setIsDialogOpen(false);
        resetForm();
        fetchDownloads();
      }
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Upload a file for students to download.</DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-4">
              ...
              <div className="flex justify-end gap-2 pt-4">
                ...
                <Button type="submit" disabled={saving || !formData.file_url}>
                  ...
                  {editingItem ? "Update" : "Add"} Download
                </Button>
              </div>
            </form>
          </DialogContent>
